...
class AnimationDebug extends FlxState
{
	...
	override function create()
	{
		...
		//dadText.cameras = [camHUD]; //for some reason, the hitbox doesn't zoom out with it
		add(dadText);

		bfText = new FlxUIInputText(1200, 60, 70, 'bf');
		bfText.text = daOtherAnim;
		bfText.scrollFactor.set();
		//bfText.cameras = [camHUD]; //for some reason, the hitbox doesn't zoom out with it
		add(bfText);
		...
		textAnim.color = 0xFF36FF5B;
		textAnim.setBorderStyle(OUTLINE, 0xFF000000, 3, 1);
		textAnim.cameras = [camHUD]; //for some reason, the hitbox is the only one that does zoom out with it
		add(textAnim);
