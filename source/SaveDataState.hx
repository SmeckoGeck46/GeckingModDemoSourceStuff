	...
	override function create()
	{
		...
		var menuBG:FlxSprite = new FlxSprite().loadGraphic('assets/images/menuDesat.png');
			optionList = [
							...
							{name: "Alternate Vocals", value: false, intName: "stressTankmen", desc: "Changes the vocals for the Eddsworld demo and VS N demo to something different, whether it be alternate voices or funny voices."}, //this is the funny voices option but with a different name
							...
							/*{name: "Allow Story Mode", value: false, intName:"allowStoryMode", desc: "Show story mode from the main menu.", ignore: false},
							{name: "Allow Freeplay", value: false, intName:"allowFreeplay", desc: "Show freeplay from the main menu.", ignore: false},*/
		...
		if (controls.ACCEPT) {
			if (saves.members[curSelected].beingSelected) {
				if (!saves.members[curSelected].askingToConfirm) {
					...
					FlxG.save.erase();
					saves.members[curSelected].askToConfirm(false);
					// it's TomSka doing an impression of the Peanut Butter Baby. perfect for a delete sfx
					FlxG.sound.play('assets/images/custom_dialogs/dialogSounds/eatYourHeartOutGawrGura.ogg');
					FlxG.save.data.songScores = ["tutorial" => 0];
