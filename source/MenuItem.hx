...
class MenuItem extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, weekNum:Int = 0)
	{
		...
		if (rawXml != "") {
			...
			week.frames = tex;
			// TUTORIAL IS WEEK 0
			week.animation.addByPrefix("default", animName + "0000", 24);
			week.animation.addByPrefix("select", animName, 16);
			add(week);
	...
	override function update(elapsed:Float)
	{
		...
		/*if (StoryMenuState.selectedWeek = true)
			week.animation.play('select');*/
	}
}
