...
class VictoryLoopState extends MusicBeatSubstate
{
		...
		switch (PlayState.SONG.song) { //different pause theme for each demo
			case "Vanity" | "Ingoobelyblench" | "Bacon-Cola" | "Edd's-Crappy-Song": //Just Another Eddsworld Mod Demo
				if (accuracy >= 65) {
					...
					FlxG.sound.playMusic('assets/music/JAEM_victory_C_120_BPM' + TitleState.soundExt);
				} else if (accuracy >= 50) {
					Conductor.changeBPM(108);
					FlxG.sound.playMusic('assets/music/JAEM_victory_B_108_BPM' + TitleState.soundExt);
				} else {
					Conductor.changeBPM(100);
					FlxG.sound.playMusic('assets/music/JAEM_victory_A_100_BPM' + TitleState.soundExt);
				}
				case 'Star-Walker' | 'Pipis-Song': //The Original Starwalker and Friends Demo
				if (accuracy >= 65) {
					Conductor.changeBPM(90); //dancing in half-time so BF and GF are in-sync
					FlxG.sound.playMusic('assets/music/starwalkerWin_2' + TitleState.soundExt); //renamed files thanks to Explorer++
				} else if (accuracy >= 50) {
					Conductor.changeBPM(55);
					FlxG.sound.playMusic('assets/music/starwalkerWin_1' + TitleState.soundExt);
				} else {
					Conductor.changeBPM(55);
					FlxG.sound.playMusic('assets/music/starwalkerWin_0' + TitleState.soundExt);
				}
			case 'Disembodied-Voice' | 'Dismantle' | 'Huni': //VS N Demo
				if (accuracy >= 65) {
					Conductor.changeBPM(125);
					FlxG.sound.playMusic('assets/music/VS_N_victory3_125_BPM' + TitleState.soundExt);
				} else if (accuracy >= 50) {
					Conductor.changeBPM(110);
					FlxG.sound.playMusic('assets/music/VS_N_victory2_110_BPM' + TitleState.soundExt);
				} else {
					Conductor.changeBPM(110);
					FlxG.sound.playMusic('assets/music/VS_N_victory1_110_BPM' + TitleState.soundExt);
				}
			case 'Sugar-and-Spice' | 'Boba' | "There's-Tord": //Friday Night Brickin with General Mayhem Demo + the bonus Tord song
				if (accuracy >= 65) {
					Conductor.changeBPM(130);
					FlxG.sound.playMusic('assets/music/SweetMayhem_victory3_130_bpm' + TitleState.soundExt);
				} else if (accuracy >= 50) {
					Conductor.changeBPM(109);
					FlxG.sound.playMusic('assets/music/SweetMayhem_victory2_109_bpm' + TitleState.soundExt);
				} else {
					Conductor.changeBPM(101);
					FlxG.sound.playMusic('assets/music/SweetMayhem_victory1_101_bpm' + TitleState.soundExt);
				}
			default: //anything else that isn't mentioned
				if (accuracy >= 65) {
	...
	override function beatHit()
	{
		...
		if (accuracy >= 0.65) {
			gf.playAnim('cheer');
			if (gf.animation.curAnim.name != 'cheer') {
				// boogie if no cheer anim, looks kinda silly
				gf.dance();
			}
